...
[t, Y] = ode45(@(t, U) odeAllStaticKB(t, U, K_B0, delta, u_max, u_alpha, beta, K_m, kcat, yCO3, kpptn), tspan, U0);
[t1,Y1] = ode45(@(t, U) odeAllDynamicKB(t, U, K_B0, delta, u_max, alpha, beta, K_m, kcat, yCO3, kpptn, p_max,u_alpha), tspan, U0);
figure
yyaxis left
...
figure
yyaxis left
plot(t1,Y1(:,5),LineWidth=2); hold on;
plot(t1,Y1(:,6), LineWidth=2);hold on;
xlabel("Time(hours");
ylabel("Amount of Ca^{2+} and CaCO_3 (mM?)");
yyaxis right
plot(t1,Y1(:,1),LineWidth=2);
ylabel("Number of cells(cells/mL)");
title("Plot of Bacteria, Calcium Ions and Calcium Carbonate Dynamics -DKB");
legend("Ca^{2+}","CaCO_3","Bacteria");
...
function dUdt = odeAllStaticKB(t, U, K_B, delta, u_max, u_alpha, beta, K_m, kcat, yCO3, kpptn)
    B = U(1); E = U(2); S = U(3); C = U(4); Ca = U(5); CaCO3 = U(6) ;

    dUdt = zeros(6,1);
    dUdt(1) = u_max * B * (1 - B / K_B) - delta * B;           % Bacteria
    dUdt(2) = (u_alpha * B) - (beta * E);                      % Urease
...
function dUdt = odeAllDynamicKB(t, U, K_B0, delta, u_max, alpha, beta, K_m, kcat, yCO3, kpptn, p_max,u_alpha)
    B = U(1); E = U(2); S = U(3); C = U(4); Ca = U(5); CaCO3 = U(6) ;
    dUdt = zeros(6,1);
    K_B = K_B0 * (1 - alpha * (CaCO3 / p_max));
    %dbdt = u_max * B * (1 - B / K_B) - delta * B;
    dUdt(1) = u_max * B * (1 - B / K_B) - delta * B;           % Bacteria
    dUdt(2) = (u_alpha * B) - (beta * E);                            % Urease
    v = (kcat * E * S) / (K_m + S);                            % Michaelis-Menten rate
